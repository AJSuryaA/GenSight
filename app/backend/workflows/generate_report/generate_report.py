...
# Streamlit app
def run_dashboard(insights):
    st.title("GenSight Data Insights Dashboard")

    st.header("EDA Overview")
    for point in insights.get("EDA_Overview", []):
        st.write("- ", point)

    st.header("Visualizations and Image Insights")
    for img_filename, insight_text in insights.get("Image_Insights", {}).items():
        image_path = os.path.join(IMAGE_FOLDER, img_filename)
        st.subheader(img_filename)
    ...
    for point in insights.get("Summary_Insights", []):
        st.write("- ", point)

    def generate_html_report(insights, image_folder):
        html = """
        <html><head><title>GenSight Data Insights Report</title></head><body>
        <h1>EDA Overview</h1><ul>"""
        for p in insights.get("EDA_Overview", []):
            html += f"<li>{p}</li>"
        html += "</ul><h1>Visualizations and Image Insights</h1>"
        for img_name, text in insights.get("Image_Insights", {}).items():
            html += f"<h3>{img_name}</h3>"
            img_path = os.path.join(image_folder, img_name)
            if os.path.isfile(img_path):
                html += f'<img src="file://{img_path}" style="max-width:600px;"><p>{text}</p>'
            else:
                html += f"<p><i>Image file not found: {img_name}</i></p><p>{text}</p>"
        html += "<h1>Summary Insights</h1><ul>"
        for p in insights.get("Summary_Insights", []):
            html += f"<li>{p}</li>"
        html += "</ul></body></html>"
        return html

    if st.button("Download Report as PDF"):
        html_str = generate_html_report(insights, IMAGE_FOLDER)
        with NamedTemporaryFile(suffix=".pdf") as pdf_file:
            HTML(string=html_str).write_pdf(pdf_file.name)
            pdf_file.seek(0)
            pdf_bytes = pdf_file.read()
            st.download_button(label="Click to download PDF", data=pdf_bytes, file_name="GenSight_Report.pdf", mime="application/pdf")
